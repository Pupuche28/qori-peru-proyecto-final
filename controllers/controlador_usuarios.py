...
def insertar_usuario(nombres, apellidos, email, telefono, documento, contrasena, direccion, rol):
    try:
        ...
        with conexion.cursor() as cursor:
            # Almacenar la contraseña directamente sin hashear
            sql = """
                INSERT INTO USUARIO (nombres, apellidos, email, telefono, nroDocIde, contrasena, direccion, idRol)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """
            cursor.execute(sql, (nombres, apellidos, email, telefono, documento, contrasena, direccion, rol))
        conexion.commit()
        print(f"Usuario {email} registrado correctamente.")  # Mensaje de éxito
...
def obtener_usuarios():
    try:
        ...
        with conexion.cursor() as cursor:
            sql = """
                SELECT u.idUsuario, u.nombres, u.apellidos, u.email, u.telefono, u.nroDocIde, u.direccion, r.nombre AS rol
                FROM USUARIO u
...
def obtener_todos_los_usuarios():
    try:
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            # Seleccionamos todas las columnas excepto la contraseña
            sql = """
                SELECT idUsuario, nombres, apellidos, email, telefono, nroDocIde, direccion, idRol 
                FROM USUARIO
            """
            cursor.execute(sql)
            usuarios = cursor.fetchall()
